...

{
  ...
  echo "| JaCoCo line coverage (merged unit + integration) | >= 90% |"
  echo "| JaCoCo branch coverage (merged unit + integration) | >= 85% |"
  ...
  echo "| Gate | Threshold |"
  echo "|---|---|"
  echo "| Trivy dependency scan (application JAR) | Fails on CRITICAL or HIGH |"
  ...
  echo "|---|---|---|"
  status_line "JaCoCo coverage (merged)" "jacoco-report"
  ...
  status_line "Trivy dependency scan" "dependency-scan-report"
